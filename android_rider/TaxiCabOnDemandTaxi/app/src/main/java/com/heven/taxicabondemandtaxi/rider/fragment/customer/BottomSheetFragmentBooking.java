...
//        cancel = (ImageView) rootView.findViewById(R.id.cancel);
        ...
        if(tabDistance[1].equals("m"))
            ...
            distance_init = String.valueOf(Float.parseFloat(tabDistance[0].replace(",",""))*1000);
...
//            intput_layout_people_number.setError(context.getResources().getString(R.string.enter_a_number_greater_than));
    ...
    private static void getStaticMap(){
        if(points.size() != 0){
            ...
            final String STATIC_MAP_API_ENDPOINT = "http://maps.googleapis.com/maps/api/staticmap?size=1000x300&markers="+loc1.getLatitude()+","+loc1.getLongitude()+"|"+loc2.getLatitude()+","+loc2.getLongitude()+"&path=color:0x000000|"+path+"&sensor=false&key=" + context.getString(R.string.google_maps_key);
